...
def train1(model, device, train_loader, optimizer):
    model.train()
...
def train2(model, device, train_loader, optimizer):
    model.train()
...
def train_and_evaluate(config, max_budget, verbose=False, **kwargs):
    ...
    for epoch in range(1, int(max_budget)+1):
        train2(model, device, train_loader, optimizer)
...
def objective_function(config, budget, **kwargs):
    ...
    for epoch in range(1, int(budget)+1):
        ...
        train1(model, device, dataloader, optimizer)
        
        if (epoch % (int)(0.1 * 100) == 0):
            dataloader = GLISTERDataLoader(train_loader, 
                                valid_loader, 
                                dss_args, 
                                logging.getLogger(), 
...
def input_arguments():
    ...
    parser.add_argument('--refit_training', action='store_true', default=True,
