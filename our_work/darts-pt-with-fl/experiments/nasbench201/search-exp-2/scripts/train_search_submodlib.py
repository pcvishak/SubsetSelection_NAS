...
parser.add_argument('--dataset', type=str, default='cifar10',
                    choices=['cifar10', 'cifar100', 'imagenet16-120'], help='choose dataset')
parser.add_argument('--method', type=str,
                    default='dirichlet', help='choose nas method')
parser.add_argument('--search_space', type=str, default='nas-bench-201')
parser.add_argument('--batch_size', type=int, default=64,
                    help='batch size for alpha')
...
parser.add_argument('--fast', action='store_true',
                    default=False, help='skip loading api which is slow')
parser.add_argument('--resume_epoch', type=int, default=0,
                    help='0: from scratch; -1: resume from latest checkpoints')
parser.add_argument('--resume_expid', type=str, default='',
                    help='e.g. search-darts-201-2')
parser.add_argument('--dev', type=str, default=None,
                    help='separate supernet traininig and projection phases')
parser.add_argument('--ckpt_interval', type=int,
                    default=20, help='frequency for ckpting')
parser.add_argument('--expid_tag', type=str, default='none',
                    help='extra tag for exp identification')
parser.add_argument('--log_tag', type=str, default='',
                    help='extra tag for log during arch projection')
# projection
parser.add_argument('--edge_decision', type=str, default='random',
                    choices=['random'], help='which edge to be projected next')
parser.add_argument('--proj_crit', type=str, default='acc',
                    choices=['loss', 'acc'], help='criteria for projection')
parser.add_argument('--proj_intv', type=int, default=5,
                    help='fine tune epochs between two projections')
################################
...
parser.add_argument('--function_name', type=str, default='',
                    help='function name for submodlib')
...
if args.resume_epoch > 0:  # do not delete dir if resume:
    args.save = '../experiments/nasbench201/{}'.format(
        args.resume_expid+args.function_name)
    if not os.path.exists(args.save):
        print('no such directory {}'.format(args.save))
else:
    scripts_to_save = glob.glob(
        '*.py') + ['../exp_scripts/{}.sh'.format(expid)]
...
if args.resume_epoch > 0:
    log_file = 'log_resume-{}_dev-{}_seed-{}_intv-{}'.format(
        args.resume_epoch, args.dev, args.seed, args.proj_intv)
    if args.log_tag != '':
...
def main():
    ...
    if args.method in ['darts', 'blank']:
        model = TinyNetworkDarts(C=args.init_channels, N=5, max_nodes=4, num_classes=n_classes,
                                 criterion=criterion, search_space=search_space, args=args)
    elif args.method in ['darts-proj', 'blank-proj']:
        model = TinyNetworkDartsProj(C=args.init_channels, N=5, max_nodes=4,
                                     num_classes=n_classes, criterion=criterion, search_space=search_space, args=args)
    model = model.cuda()
    ...
    # if args.random:
    #     indices = get_proxy_data_random(
    #         entropy, args.sampling_portion, logging)
    # elif args.histogram:
    #     indices = get_proxy_data_log_entropy_histogram(
    #         entropy, args.sampling_portion, args.histogram_type, args.dataset, logging)
    # else:
    ...
    # if ( args.function_name == "facility_location"):

    data = np.load('input.npy')
    groundData = np.reshape(data, (data.shape[0], -1))
    groundShape = data.shape[0]
    subsetsize = int(groundShape * args.sample_portion)

    if (args.function_name == "disparity_sum"):
        objDM = DisparitySumFunction(
            n=groundShape, data=groundData, mode="dense", metric="euclidean")
        greedyList = objDM.maximize(budget=subsetsize, optimizer='NaiveGreedy',
                                    stopIfZeroGain=False, stopIfNegativeGain=False, verbose=False)
    elif(args.function_name == "graph_cut"):
        objGC = GraphCutFunction(n=groundShape, mode="dense", separate_rep=False,
                                 lambdaVal=1, data=groundData, metric="euclidean")
        greedyList = objGC.maximize(budget=subsetsize, optimizer='NaiveGreedy',
                                    stopIfZeroGain=False, stopIfNegativeGain=False, verbose=False)
    elif(args.function_name == "disparity_min"):
        objDM = DisparityMinFunction(
            n=groundShape, data=groundData, mode="dense", metric="euclidean")
        greedyList = objDM.maximize(budget=subsetsize, optimizer='NaiveGreedy',
                                    stopIfZeroGain=False, stopIfNegativeGain=False, verbose=False)
    elif(args.function_name == "log_determinant"):
        objLogDet = LogDeterminantFunction(n=groundShape, data=groundData, mode="dense",
                                           metric="euclidean", lambdaVal=0.1)
        greedyList = objLogDet.maximize(budget=subsetsize, optimizer='NaiveGreedy',
                                        stopIfZeroGain=False, stopIfNegativeGain=False, verbose=False)
    # print(greedyList)
    # logging.info(greedyList)
    indices = [x[0] for x in greedyList]
    ...
    num_train = subsetsize

    with open(os.path.join(args.save, args.function_name + 'subset_indices_train_file.txt'), 'w') as f:
        for idx in indices[:split]:
            f.write('%d %f %d\n' % (idx, label[idx]))
            num_classes[label[idx]] += 1
    with open(os.path.join(args.save, args.function_name + 'subset_indices_val_file.txt'), 'w') as f:
        for idx in indices[split:num_train]:
            f.write('%d %f %d\n' % (idx, label[idx]))
            num_classes[label[idx]] += 1
    logging.info(num_classes)
    ...
    if args.resume_epoch != 0:
        ...
        file = 'checkpoint.pth.tar' if args.resume_epoch == - \
            1 else 'checkpoint_{}.pth.tar'.format(args.resume_epoch)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_acc, train_obj = train(
            train_queue, valid_queue, model, architect, model.optimizer, lr, epoch)
        ...
        if not args.fast:
            ...
            cifar10_train, cifar10_test, cifar100_train, cifar100_valid, \
                cifar100_test, imagenet16_train, imagenet16_valid, imagenet16_test = query(
                    api, model.genotype(), logging)

            # tensorboard
            writer.add_scalars(
                'accuracy', {'train': train_acc, 'valid': valid_acc}, epoch)
            writer.add_scalars(
                'loss', {'train': train_obj, 'valid': valid_obj}, epoch)
            writer.add_scalars(
                'nasbench201/cifar10', {'train': cifar10_train, 'test': cifar10_test}, epoch)
        ...
        if save_state['epoch'] % args.ckpt_interval == 0:
            ig_utils.save_checkpoint(
                save_state, False, args.save, per_epoch=True)
...
def query(api, genotype, logging):
    ...
    logging.info('cifar100 train %f valid %f test %f',
                 cifar100_train, cifar100_valid, cifar100_test)
