...
def main():
    ...
    data = np.array([ele[0] for ele in train_data])
    ...
    for i in range(groundData.shape[0]):
        label_data[label[i]].append((groundData[i],i))
    
    indices = []
    for i in range(num_labels):
        objFL = FacilityLocationFunction(n=len(label_data[i]), data=np.array([ele[0] for ele in label_data[i]]), separate_rep=False, mode="dense", metric="euclidean")
        greedyList = objFL.maximize(budget=subsetsize/num_labels,optimizer='NaiveGreedy', stopIfZeroGain=False, stopIfNegativeGain=False, verbose=False)
        indices.extend([label_data[i][x[0]][1] for x in greedyList])
    
    indices.sort()

    num_train = subsetsize
    class_freq = [0] * (n_classes)

    with open(os.path.join(args.save, 'fl_indices_train_file.txt'), 'w') as f:
        for idx in indices[:split]:
            f.write('%d %d\n' % (idx, label[idx]))
            class_freq[label[idx]] += 1
    with open(os.path.join(args.save, 'fl_indices_val_file.txt'), 'w') as f:
        for idx in indices[split:num_train]:
            f.write('%d %d\n' % (idx, label[idx]))
            class_freq[label[idx]] += 1
    logging.info('class frequency array = %s', str(class_freq)) #just to check if each class has equal representation in the final dataset
