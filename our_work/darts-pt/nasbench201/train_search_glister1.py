...
from nasbench201.glisterstrategy import GLISTERStrategy
...
def main():
    ...
    split = int(np.floor(args.train_portion * len(train_data)))
...
def train(train_queue, valid_queue, model, architect, optimizer, lr, epoch):
    ...
    for step in range(len(train_queue)):
        ...
        input_search = input_search.to(device); target_search = target_search.to(device)
